	...
	for epoch in range(flags.n_epoch):
		for step, batch_images in enumerate(images):
			...
			with tf.GradientTape(persistent=True) as tape:
				...
				d_logits = D(G(z))
				cv2.imshow('My Image', np.array(batch_images[0], dtype="float"))
				cv2.waitKey(30)
		...
		if np.mod(epoch, flags.save_every_epoch) == 0:
			G.save_weights('{}/G{:02d}.npz'.format(flags.checkpoint_dir, epoch), format='npz')
			D.save_weights('{}/D{:02d}.npz'.format(flags.checkpoint_dir, epoch), format='npz')
			G.eval()
			result = G(z)
			G.train()
